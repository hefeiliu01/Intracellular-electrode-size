...
height = 1e-6; % pillar height 2 um
electrode_surface_area = 2 * pi * relectrode * height + pi * relectrode.^2;

Deq = sqrt(electrode_surface_area / pi) * 2;
...
Ze0 = 20e-6; % set from 5e-6 to 500e-6
...
figure(1)
%plot(relectrode * 2, mean_alpha)
%xlabel('electrode diameter (m), cell diameter = 20 um')
plot(Deq, mean_alpha)
xlabel('equivalent disk diameter (m), cell diameter = 20 um')
ylabel('mean alpha')
set(gca, 'XScale', 'log')
%title('Ze ~ 1/A')
title('Ze0 = 20 MOhm*um^2, Rs = 100 MOhm, Zjm = 400 MOhm')
legend('disk','hole')
hold on;
